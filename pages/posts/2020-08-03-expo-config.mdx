...
Expoではアプリのメタ情報や設定値を`app.json`または`app.config.json`に記載します。ただ`app.json`はstaticなJSONファイルのためそのままだと環境別(例えばdevelopment, staging, productionのように)に値を指定できません。
...
Expo Updateでなく新しい形式のEAS Updateを使っている場合、 `Constants.manifest`はnullになってしまうため、`Constants.manifest2` を使います。

```ts
import Constants from 'expo-constants'

console.log("API_ENDPOINT:", Constants.manifest2.extra.expoClient.extra.API_ENDPOINT)
```
