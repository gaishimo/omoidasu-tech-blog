...
TestFlightでの配布が可能になったことで内部配布を使うメリットはあまり無くなるかもしれません。通常のアプリのテストも基本的にはTestFlightを通せばよいと思います。ただTestFlightの内部テストはTeamに参加しているメンバーでないとできず、パブリックの外部テストは審査が必要になるため、配布したいユーザが多い場合は内部配布を使うメリットがありそうです。

また、本番用の通常のアプリをTestFlightでテストする際に開発用ビルドと混在してしまうと困るため、開発用と本番用はbundleIdを分けて別アプリとしておいたほうが良さそうに思いました。
