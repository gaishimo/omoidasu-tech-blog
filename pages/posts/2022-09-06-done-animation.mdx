...
今回はチェックマークの線を2つのPathに分けて1本ずつ順番にアニメーションしていきます。1つのPathで描くことも可能ですが、その場合線の長さの計算が少し複雑になるため今回はあえて分けます。(線が多い場合や線を最終的につなげる場合は1つのPathで行った方がコントロールしやすいと思います)

線を引くため現在のアニメーション進捗に応じて線の終点を計算します。まず1つ目のポイントから2つ目のポイントまでどれだけ移動したかベクトルを取得します。
...
取得したベクトルのxとyに対して現在の進捗比率をかけることにより、進捗に応じた長さにすることができます。
```ts
...
アニメーションを少し調整してみます。Easing関数を指定するのと、1本目の線と2本目の線の間にわずかにインターバルを空けています。微妙に変化が異なるのが分かると思います。
...
処理待ちのときに変化するボタン等に使ったりもできそうです。


コードは[こちら](https://github.com/gaishimo/omoidasu-tech-blog/tree/main/components/pages/2022-09-06-done-animation)で確認できます。
